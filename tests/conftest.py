...
def _require_database_url() -> URL:
    ...
    if not database_url:
        raise RuntimeError("DATABASE_URL must be set for the Postgres test suite")

    url = make_url(database_url)
    if not url.drivername.startswith("postgresql"):
        raise RuntimeError(f"Postgres test suite requires a PostgreSQL DATABASE_URL, got {url.drivername}")
    if url.database != TEST_DATABASE_NAME:
        raise RuntimeError(
            f"Postgres test suite requires DATABASE_URL to point to {TEST_DATABASE_NAME}, got {url.database}"
        )
    return url


def _sync_database_url(url: URL) -> str:
...
def postgres_test_database() -> str:
    ...
    with psycopg.connect(_admin_database_url(url), autocommit=True) as connection:
        connection.execute(sql.SQL("DROP DATABASE IF EXISTS {} WITH (FORCE)").format(sql.Identifier(database_name)))
        connection.execute(sql.SQL("CREATE DATABASE {}").format(sql.Identifier(database_name)))

    alembic_config = Config("alembic.ini")
    alembic_config.set_main_option("sqlalchemy.url", _sync_database_url(url))
